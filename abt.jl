### A Pluto.jl notebook ###
# v0.14.5

using Markdown
using InteractiveUtils

# ╔═╡ e34aafc0-b721-11eb-0e8f-1b876bbda906
begin
	using Turing
	using Random
	using MCMCChains, ArviZ, Serialization, PyPlot
end

# ╔═╡ 1a862514-dca4-4eb7-9236-e80f2a27eef4
include("softmax.jl")

# ╔═╡ 951e2ad2-72db-459e-bf47-ff3875182bba
include("lapse.jl")

# ╔═╡ a05e60a2-d069-46db-bf8e-9f2feb06e28b
include("read_abt.jl")

# ╔═╡ 92d44759-5a63-464d-bc2f-1ba2cc2221ec
struct ABT_t
	n_sessions::Int64
	n_subjects::Int64
	n_interv::Int64
	avail_actions_m::Matrix{Array{Int64,1}}
	interv_m::Matrix{Int64}
	R_m::Matrix{Array{Float64,1}}
	trial_m::Matrix{Int64}
end

# ╔═╡ 356db041-eba4-4d10-bb81-5c6d38e1a9f8
ArviZ.use_style("arviz-darkgrid")

# ╔═╡ 4d6eed54-2db2-41ce-853e-6b2304b7d5d8
chn = deserialize("chn_softmax_FG.jls");

# ╔═╡ c49300cc-123d-4867-9114-4348619e140c
idt_softmax = from_mcmcchains(chn; 
							coords = Dict("interv" => ["V","FG_3","FG_6"],
										"subj" => [i for i in 1:12]),
							dims = Dict("μ_β_v" => ["interv"], 
										"σ_β_v" => ["interv"],
										"μ_η_v" => ["interv"], 
										"σ_η_v" => ["interv"],
										"β_norm_m" =>["interv", "subj"],
										"η_norm_m" =>["interv", "subj"]),
							library="Turing")

# ╔═╡ 0c7096c8-9cb8-42a1-9a41-e0eff5884305
begin
	plot_trace(idt_softmax)
	gcf()
end

# ╔═╡ b97d7720-86dc-4b7d-9fb8-d72f616a6164
begin
	plot_energy(idt_softmax)
	gcf()
end

# ╔═╡ ac16ab02-e131-4ac6-8699-4b6d7f8e69dd
begin
	plot_posterior(chn; var_names = ["σ_β_v"])
	gcf()
end

# ╔═╡ 0f7b1b7d-d1ae-412d-b4f6-fb0cd7b11c19
begin
	
	file_v = ["./abt/ER17_FG7142_trials.csv", 
				"./abt/ER17_2vs1_trials.csv"]
	cb_file_v =["./abt/ER17_FG7142_counterbalance.csv", 			
				"./abt/ER17_2vs1_counterbalance.csv"]

	(action_m, data) = read_data(file_v, cb_file_v)
	
	model_prior = softmax_model(action_m, data)
	
	prior = sample(model_prior, Prior(), 1000; progress=false)
end

# ╔═╡ cffce24f-80d9-49d4-8a37-c33432b5e147
model_predict = softmax_model(missing, data)

# ╔═╡ c2c65caf-8153-4c27-befe-bdc985ffa479
prior_predictive = predict(model_predict, prior)

# ╔═╡ db9c7c58-4ec2-4bad-bd6c-55b47e7a8d23
posterior_predictive = predict(model_predict, chn)

# ╔═╡ 89571289-23e5-4deb-8bd9-d22b60683929
loglikelihoods = Turing.pointwise_loglikelihoods(
    			model_prior, MCMCChains.get_sections(chn, :parameters))

# ╔═╡ Cell order:
# ╠═e34aafc0-b721-11eb-0e8f-1b876bbda906
# ╠═92d44759-5a63-464d-bc2f-1ba2cc2221ec
# ╠═1a862514-dca4-4eb7-9236-e80f2a27eef4
# ╠═951e2ad2-72db-459e-bf47-ff3875182bba
# ╠═a05e60a2-d069-46db-bf8e-9f2feb06e28b
# ╠═356db041-eba4-4d10-bb81-5c6d38e1a9f8
# ╠═4d6eed54-2db2-41ce-853e-6b2304b7d5d8
# ╠═c49300cc-123d-4867-9114-4348619e140c
# ╠═0c7096c8-9cb8-42a1-9a41-e0eff5884305
# ╠═b97d7720-86dc-4b7d-9fb8-d72f616a6164
# ╠═ac16ab02-e131-4ac6-8699-4b6d7f8e69dd
# ╠═0f7b1b7d-d1ae-412d-b4f6-fb0cd7b11c19
# ╠═cffce24f-80d9-49d4-8a37-c33432b5e147
# ╠═c2c65caf-8153-4c27-befe-bdc985ffa479
# ╠═db9c7c58-4ec2-4bad-bd6c-55b47e7a8d23
# ╠═89571289-23e5-4deb-8bd9-d22b60683929
