...
function P_ε_greedy(ε, r_v)
	...
	if !all(x -> x == r_v[1], r_v)
		...
		return [ε / n_actions for _ = 1:n_actions] + (1.0 - ε) .* v
	else
		return [1.0 / n_actions for _ = 1:n_actions]
	end
end

@model function ε_greedy_2_model(choice_m, data::ABT_t, ::Type{T} = Float64) where {T <: Real}
	...
	for subject = 1 : data.n_subjects
		...
		for session = 1 : data.n_sessions
			...
			for trial = 1 : data.trial_m[subject, session]
				...
				P_v = P_ε_greedy(ε, r_v[avail_actions_v])
...
@model function ε_greedy_3_model(choice_m, data::ABT_t, ::Type{T} = Float64) where {T <: Real}
...
function predict_ε_greedy_3(choice_m, data, chn)
